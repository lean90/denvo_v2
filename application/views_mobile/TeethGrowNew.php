...
<div class="profile-box profile-box-add form-horizontal" ng-show="onCreateNewProfile">
	...
	<div class="form-group col-md-10">
		...
		<div class="col-sm-9">
			<input id="email" ng-model="createNewProfile.email" type="email"
    	...
    	<div class="schema-conatiner">
    		<ul ng-click="selectedTeeth($event, teethChildPosition)"
    			...
    			<li
    				...
    				ng-repeat="teeth in createNewProfile.history"
    				style="background: url(/schema/{{teeth.code}}.png)"></li>
    		</ul>
    		<div class="schema-description">
    			<div ng-click="openToolTip('description-new-1')"> <span class="profile-title-higlight">Sơ đồ răng sữa</span> <i class="fa fa-question-circle profile-title-higlight"></i></div>
				<div id="description-new-1" style="display: none;color: #b3b3b3">
    		...
    		<div class="table-container child-table">
    			<table class="table table-striped table-bordered"
    				ng-show="getDisplayTimelineTableData(createNewProfile.history,'RS') != 0">
    				<thead>
    					<tr>
    						<th>Tên răng</th>
    						<th>Thời gian mọc</th>
    				...
    				<tbody>
    					<tr
    						ng-repeat="teeth in getDisplayTimelineTableData(createNewProfile.history,'RS')">
    						<td>{{teeth.code}}</td>
    						<td>{{teeth.current}}</td>
    						<td>{{teeth.normal}}</td>
	...
	<div class="col-md-12 " style="margin-top: 15px">
		<div class="schema-conatiner">
			...
			<div class="schema-description">
				<div ng-click="openToolTip('description-new-2')"> <span class="profile-title-higlight">Sơ đồ răng vĩnh viễn</span> <i class="fa fa-question-circle profile-title-higlight"></i></div>
			    <div id="description-new-2" style="display: none;color: #b3b3b3">
    				Chia 4 cung, theo chiều kim đồng hồ<br />
		...
		<div class="table-container man-table">
			<table class="table table-striped table-bordered"
				ng-show="getDisplayTimelineTableData(createNewProfile.history,'RVV').length != 0">
				<thead>
					<tr>
						<th>Tên răng</th>
						<th>Thời gian mọc</th>
				...
				<tbody>
					<tr
						ng-repeat="teeth in getDisplayTimelineTableData(createNewProfile.history,'RVV')">
						<td>{{teeth.code}}</td>
						<td>{{teeth.current}}</td>
						<td>{{teeth.normal}}</td>
					</tr>
			...
			<div class="modal-body" style="display: inline-block; padding: 0px">
				<table class="frm-teethGrow" style="width: 470px; margin-top: 10px;">
					...
					<tr ng-show="editingTeeth.code.indexOf('RS') != -1">
						...
						<td><select ng-model="editingTeeth.rcurrent"
							ng-show="editingTeeth.code.indexOf('RS') != -1">
